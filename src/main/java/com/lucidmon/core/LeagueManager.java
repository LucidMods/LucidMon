...
/**
 * Unstable-3 server control surface.
...
public final class LeagueManager {
    ...
    public static int cmdStatus(Object ctx,Object src){
        ...
        CommandBridge.feedback(src,"Active test modules: Kanto MapGen, gym placement commands, map reveal, soulpack, virtual pasture. Player Champion battle flow is scaffold-only.","yellow");
        return 1;
    }
    ...
    private static int placeGym(String id,Object src){
        ...
        if(g.rotation()!=ConfigManager.Rotation.NONE) CommandBridge.feedback(src,"UNSTABLE.3 note: RGS /place structure does not apply configured rotation yet; placing with NONE orientation.","yellow");
    ...
    public static int cmdMapGenPreflight(Object ctx,Object src){
        ...
        if(m.mapType()==MapGenConfigManager.MapType.KANTO_ARCHIPELAGO){
            ...
            CommandBridge.feedback(src,"Profile values: startCity="+k.startingCityStructure()+" @ "+k.startingCityX()+","+k.startingCityZ()+" | volcano="+k.volcanoX()+","+k.volcanoZ()+" | MtMoon="+k.mtMoonX()+","+k.mtMoonZ(),"aqua");
            if(!KantoMapGen.isActive()){
                CommandBridge.feedback(src,"BLOCKER: Kanto world preset is not active. For a NEW dedicated-server world set level-type="+KantoMapGen.WORLD_PRESET_ID+" in server.properties, then restart.","red");
                return 0;
            }
            CommandBridge.feedback(src,"KANTO pre-flight PASS: generator active, profile lock verified, boundary policy loaded.","green");
            return 1;
        }
    ...
    public static int cmdMapGenAuditBiomes(Object ctx,Object src){
        ...
        CommandBridge.feedback(src,"Kanto generator is active. Full 53-biome generated-world audit scanning is not yet implemented in unstable.3; use this build for terrain/layout verification first.","yellow");
        return 0;
    }
    ...
    private static int championPending(Object src){CommandBridge.feedback(src,"Player Champion queue/battle integration is not enabled in v0.1.2-unstable.3. The JSON5 section remains parsed for forward compatibility.","yellow");return 0;}
